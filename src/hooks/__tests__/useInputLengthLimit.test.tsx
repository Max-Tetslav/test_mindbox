...
describe(useInputLengthLimit, () => {
    it('Возвращает true если лимит превышен', () => {
        const { result } = renderHook(() => useInputLengthLimit({ inputLength: TODO_SYMBOLS_LIMIT + 1 }));

        expect(result.current.isInputLimitReached).toBe(true);
    });

    it('Возвращает false если длинна строки равна лимиту', () => {
        const { result } = renderHook(() => useInputLengthLimit({ inputLength: TODO_SYMBOLS_LIMIT }));
